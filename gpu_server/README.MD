...
Запускается на втором сервере архитектуры - GPU. 

Сервис ожидает сообщение от CPU сервера (через Celery), выполняет скоринг, результат записывает в БД обновляя лог по ключу, который генерируется на CPU сервере в момент запроса пользовтеля по endpoint **/predict**. 

Связи сервиса GPU с другими объектами архитектуры:
**/predict** (CPU) >> RabbitMQ (CPU) >> GPU >> PSQL
...
**main.py** - скрипт для запуска сервиса на GPU сервере.

**model.py** - Модель для скоринга новостной статьи

**model_inference.py** - Архитектура инференса модели
