...
В рамках текущего репозитория выполнена практическая часть ВКР по созданию (и развертыванию) сервиса направленного на идентификацию ИИ сгенерированных политических новостей. Здесть представленны ноутбуки по сбору, обработке данных, построению алгоритмов, развертыванию сервиса и все остальное, связанное с задачей ВКР. А так же скрипты для сервисной части: cpu_server + gpu_server, эти 2 сервера связанны между собой и создают единую архитектуру.
...
**cpu_server** Конфиги, скрипты, модель и прочее необходимое для поднятия сервисной части на GPU сервере (ML сервис детекции).
